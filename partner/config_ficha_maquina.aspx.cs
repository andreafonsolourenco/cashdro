...
public partial class config_ficha_maquina : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static string getCustomerList(string id)
    {
        string sql = "", ret = "";
        ...
        sql = string.Format(@"  DECLARE @id int={0}
                                SELECT
                                    ...
                                    nome,
                                    tipo
                                FROM CASHDRO_REPORT_CLIENTESPARCEIRO(@id)
                                order by tipo, nome", id);
        DataSet oDs = oDB.GetDataSet(sql, "").oData;
    ...
    public static string getUserType(string id)
    {
        ...
        if (oDB.validaDataSet(oDs) && oDs.Tables[0].Rows[0]["tipo"].ToString().Trim() == "Parceiro")
        {
            return "-1";
        }
    ...
    private void getListaClientes(string id)
    {
        string sql = "", ret = "";
