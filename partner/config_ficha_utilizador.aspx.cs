    ...
    private void getTipoUtilizadores()
    {
        ...
        if(idUser == "null" || String.IsNullOrEmpty(idUser))
        {
            sql = String.Format(@"  declare @id int;
				                    ...
				                    and nome not in ('Técnico Helptech', 'Parceiro', 'Cliente')
        ...
        else
        {
            sql = String.Format(@"  declare @id_user int = {0};
                                    ...
                                    select rpt_tp.id, rpt_tp.nome, rpt_tp.administrador
                                    from [CASHDRO_REPORT_UTILIZADORES](@id_user, @username, @password, @ativo) rpt_user
                                    inner join cashdro_report_tipo_utilizadores(@id_tipo) rpt_tp on rpt_tp.id = rpt_user.id_tipo_utilizador", idUser);
        }
